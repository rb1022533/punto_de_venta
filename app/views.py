...
def cerrar_sesion(request):
    logout(request)
    return redirect('app/iniciar_sesion')
 
#login (singin) Entrar a la sesión ya creada.
def iniciar_sesion(request):
    if request.method == 'GET':
      return render(request, 'app/iniciar_sesion.html', {'form': AuthenticationForm})
    else: 
        user = authenticate(request, username=request.POST['username'], password=request.POST['password'])
        if user is None:
         return render(request, 'app/iniciar_sesion.html', {'form': AuthenticationForm, 'error': 'Usuario y/o contraseña incorrecto.'})   
        else:
            login(request, user)
...
def agregar_venta(request):
    ...
    return render(request, 'app/agregar_venta.html', {
...
def lista_ventas(request):
    ...
    return render(request, 'app/lista_ventas.html', {'ventas': ventas})
...
def detalles_venta(request, venta_id):
    ...
    return render(request, 'app/detalles_venta.html', {
        'venta': venta,
